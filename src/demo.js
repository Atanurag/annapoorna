...
const App = () => {
  ...
  function verifyOtp(phone, otp) {
    ...
    fetch('https://annapoorna.snazzy.live/api/v1/mob_app/mobile-otp-verification/', {
      ...
      body: JSON.stringify({ mobile_number: phone, otp: otp.join('') })
    })
      .then(response => response.json())
      .then(data => {
        if (data.status === 'success') {
          localStorage.setItem('isVerified', JSON.stringify({ verified: true, timestamp: Date.now() }));
  ...
  React.useEffect(() => {
    const storedData = localStorage.getItem('isVerified');
    if (storedData) {
      const { timestamp } = JSON.parse(storedData);
      if (Date.now() - timestamp > expireTime) {
        localStorage.removeItem('isVerified')
        console.log('Local storage data expired and deleted!');
      } else {
        // Schedule the expiration function to run after 30 minutes
        const timeoutId = setTimeout(() => {
          localStorage.removeItem('isVerified');
          console.log('Local storage data expired and deleted!');
        }, expireTime - (Date.now() - timestamp));
...
<a href="upi://pay?7875853859@pthdfc&pn=Anurag%20Tiwari&mc=5812&tr=yrhty464dd&tn=Purchase%20in%20Merchant&am=1.00&cu=INR&url=https://mystar.co
            ...
            <div className='main-page'>
              <Header className='header'>
                <Row justify="space-between" align="middle" gutter={[16, 16]} >
                  <Col xs={17} sm={6} md={8} lg={12} style={{ marginBottom: '12px' }} >
                  ...
                  <Col xs={7} sm={7} md={4} lg={4} style={{ padding: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div className='switch-box'>
                        <span>Veg</span>
                        <Switch size="small" value={isVeg} onChange={(e) => {
                          setIsVeg(e)
                        }} />
...
showPhoneNo
              ...
              <div className={`bottom-anchor ${phoneVerifyBox ? 'push-to-top' : 'push-to-bottom'}`}>
                <CloseOutlined onClick={() => {
                  setPhoneVerifyBox(false);
                }} style={{ position: 'absolute', right: '25px', top: '25px', fontSize: '20px' }} />
...
<p  style={{width:'45px',fontSize:'14px'}}>₹ 1</p>
              ...
              {/* {cart} start*/}
              {(selFood?.length > 0 && paymentState?.status == '' && !phoneVerifyBox) &&
