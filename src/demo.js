...
function verifyOtp(phone,otp){
  ...
  body: JSON.stringify({ mobile_number: phone,otp: otp })
})
.then(response => response.json())
.then(data => {
if(data.status === 'success')
...
<Header className='header'>
      <Row justify="space-between" align="middle" gutter={[16,16]} >
        <Col xs={17} sm={6} md={8} lg={12} style={{marginTop:'12px'}} >
          ...
          <div className="logo-container" onClick={(event)=>{
...
<Input placeholder='Phone Number' ref={phoneRef} style={{':focus': {
      borderColor: 'blue',
      boxShadow: '0 0 0 1px blue'
    },   textAlign:'center'}}  onKeyDown={(event) => {
